...
@section('content')
    ...
    <h1>Role Matrix <small class="hide-for-small-only">Permissions that are on each role</small>
    </h1>
    ...
    {!! Form::open( [ 'route' => [ config('watchtower.route.as') .'role.matrix' ]] ) !!}
    <div class="table" style="max-height:350px; overflow:auto; border: 1px dashed;">
        <table class="hover" style=" margin-bottom:0">
            <thead>
            <tr class="alert-warning">
                <th class="text-center">
                    <span class="float-left"><span class="sr-only">Permissions</span>
                      <i class="fa fa-arrow-down"></i>
                      <i class="fa fa-key fa-lg"></i>
                    </span>

                    <span class="float-right"><span class="sr-only">Roles</span>
    ...
    @if ( Shinobi::can( config('watchtower.acl.role.rolematrix', false) ) )
        <div class="form-group">
            <div class="small-3 columns">
                {!! Form::submit('Save Role Permission Changes', ['class' => 'button']) !!}
            </div>
        </div>
    @else
