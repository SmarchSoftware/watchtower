            ...
            @foreach($users as $u)
                <tr>
                    <th class="alert-warning">
                        <a href="{{ route('watchtower.user.show',$u->user_id) }}">
                            <button type="button" class="btn btn-warning btn-xs"><span class="glyphicon glyphicon-link"></span></button></a>
                        {{ $u->login }}
                    </th>
                    @for ($i=0; $i < $roles->count(); $i++ )
                        <td data-container="body" data-trigger="focus" data-toggle="popover" data-placement="left" data-content="Role: {{$roles[$i]->name}}, User: {{$u->login}}">
                            {!! Form::checkbox('role_user[]', $roles[$i]->id.":".$u->user_id, ( in_array( ($roles[$i]->id.":".$u->user_id), $pivot ) ? true : false ) ) !!}
                        </td>
                    @endfor
                </tr>
                @endforeach
